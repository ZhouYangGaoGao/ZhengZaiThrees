...
public class GeneralTool {
    ...
    /**
     * @param edtText        目标文件
     * @param bottomDrawable 图片的位置
     * @param aty            上下文环境
     * @return
     */
    public static Drawable setNearDrawable(int bottomDrawable, Activity aty) {
        Drawable errorIcon = aty.getResources().getDrawable(bottomDrawable);
        if (errorIcon!=null) {
            errorIcon.setBounds(0, 0, errorIcon.getMinimumWidth(),
                    errorIcon.getMinimumHeight());
        }
        return errorIcon;
    }
    ...
    /**
     * @param context
     * @param Action    接收方的的 注册的 广播key ，也是传值得 key
     * @param actionKey 处理的指令
     */
     ...
     * @param callBack 借口回调
     */
    public static void uploadFile(String picFile, SaveCallback callBack) {
        String picname = picFile.substring(picFile.lastIndexOf("/") + 1);
        OSSBucket sampleBucket;
        // 开始单个Bucket的设置
        sampleBucket = new OSSBucket("pic-zhengzai-tv");
        // sampleBucket.setBucketHostId("pic-zhengzai-tv"); //
        // 可以在这里设置数据中心域名或者cname域名
        sampleBucket.setBucketHostId("oss-cn-qingdao.aliyuncs.com"); // 可以在这里设置数据中心域名或者cname域名
        // sampleBucket.setBucketACL(AccessControlList.PUBLIC_READ_WRITE); //
        // 如果这个Bucket跟全局默认的权限设置不一致
        sampleBucket.setBucketACL(AccessControlList.PRIVATE);
        OSSFile ossFile = new OSSFile(sampleBucket, "img_"
                + System.currentTimeMillis() + "_" + picname);
